...
export default function BudgetVsActualChart({
  ...
  const chartData = useMemo(
    () => transformBudgetToCategoryData(budget),
    [budget],
  );
  ...
  useEffect(() => {
    ...
    const maxValue =
      d3.max(chartData, (d) => Math.max(d.planned, d.actual)) || 0;
    const xScale = d3
      .scaleLinear()
    ...
    barGroups
      ...
      .on("mouseenter", function (event, d) {
        ...
        const hoverColor =
          d.actual > d.planned
            ? "#b91c1c"
            : d3.rgb(currentColor).darker(0.3).toString();
        ...
        setTooltip({
          ...
          content: (
            <div>
              ...
              <div
                className={`text-sm ${difference > 0 ? "text-danger" : difference < 0 ? "text-success" : "text-text-tertiary"}`}
              >
                {diffText}
              </div>
            </div>
