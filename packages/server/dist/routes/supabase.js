...
import { loadSupabaseConfig, saveSupabaseConfig, isSupabaseEnabled, maskKey, } from '@budget-app/shared/db/supabase-config';
import { testSupabaseConnection, resetSupabaseConnection, getSupabaseSQL, } from '@budget-app/shared/db/supabase-client';
...
import { getPendingChanges, getPendingCount } from '@budget-app/shared/db/sync-changelog';
...
route.get('/audit', async (c) => {
    try {
        ...
        const localClient = localDb._.session.client;
        const entries = await getPendingChanges();
        const conflicts = [];
        // Check errored entries for conflicts
        const erroredEntries = entries.filter((e) => e.error_message);
        for (const entry of erroredEntries) {
            const localRow = await localClient.query(`SELECT * FROM ${entry.table_name} WHERE id = $1`, [entry.record_id]);
            let remoteRow = null;
            try {
...
route.post('/resolve-conflict', async (c) => {
    try {
        ...
        if (strategy === 'discard') {
            ...
            await localClient.query(`UPDATE sync_changelog SET synced = true, error_message = NULL WHERE id = $1`, [changelogId]);
        ...
        if (strategy === 'use-remote') {
            ...
            const entryResult = await localClient.query(`SELECT * FROM sync_changelog WHERE id = $1`, [changelogId]);
      ...
      `;
        ...
        if (strategy === 'keep-local') {
            ...
            await localClient.query(`UPDATE sync_changelog SET error_message = NULL, last_attempt_at = NULL WHERE id = $1`, [changelogId]);
