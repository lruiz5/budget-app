...
async function verifyBudgetItemOwnership(budgetItemId, userId) {
...
async function verifyTransactionOwnership(transactionId, userId) {
    const db = await getDb();
...
route.put('/', async (c) => {
    ...
    if (linkedAccountId !== undefined)
        updateData.linkedAccountId = linkedAccountId || null;
    if (date !== undefined)
...
route.post('/batch-assign', async (c) => {
    ...
    const ownedBudgetItemIds = new Set(budgetItemsResult
    ...
    const ownedTransactionIds = new Set(txnsResult
    ...
    const validAssignments = assignments.filter(a => ownedTransactionIds.has(a.transactionId) && ownedBudgetItemIds.has(a.budgetItemId));
