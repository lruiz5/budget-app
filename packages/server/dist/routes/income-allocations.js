...
route.post('/', async (c) => {
    ...
    const existing = await db.query.incomeAllocations.findFirst({
        where: and(eq(incomeAllocations.userId, userId), eq(incomeAllocations.incomeItemName, incomeItemName)),
    });
    if (existing) {
...
route.delete('/:id', async (c) => {
    ...
    const allocation = await db.query.incomeAllocations.findFirst({
        where: and(eq(incomeAllocations.id, id), eq(incomeAllocations.userId, userId)),
    });
    if (!allocation) {
