...
route.post('/accounts', async (c) => {
    try {
        ...
        const tellerAccounts = await tellerClient.listAccounts();
    ...
    try {
        ...
        if (accountId) {
            accountsToSync = await db
                .select()
                .from(linkedAccounts)
                .where(and(eq(linkedAccounts.id, accountId), eq(linkedAccounts.userId, userId), eq(linkedAccounts.accountSource, 'teller')));
        }
        else {
            accountsToSync = await db.select().from(linkedAccounts).where(and(eq(linkedAccounts.userId, userId), eq(linkedAccounts.accountSource, 'teller')));
        }
        ...
        for (const account of accountsToSync) {
            ...
            try {
                const tellerClient = createTellerClient(account.accessToken);
                const tellerTransactions = await tellerClient.listTransactions(account.tellerAccountId, {
                    count: 500,
                    startDate,
                    endDate,
                });
...
route.get('/sync', async (c) => {
    try {
        ...
        const uncategorizedTransactions = await db.query.transactions.findMany({
            where: and(isNull(transactions.budgetItemId), isNull(transactions.deletedAt), splitParentIdList.length > 0
        ...
        const userTransactions = uncategorizedTransactions.filter(txn => txn.linkedAccount && userAccountIds.includes(txn.linkedAccount.id));
        // Look up merchant-based suggestions from historical categorizations
        const merchantNames = [...new Set(userTransactions.map(t => t.merchant).filter(Boolean))];
        ...
        if (merchantNames.length > 0) {
            const historicalTxns = await db
                ...
                .where(and(isNotNull(transactions.budgetItemId), isNull(transactions.deletedAt), inArray(transactions.merchant, merchantNames)));
            // Count frequency of each merchant -> (itemName, categoryType) pairing
            const merchantItemCounts = {};
            for (const t of historicalTxns) {
                const m = t.merchant;
                const key = `${t.categoryType}|${t.itemName}`;
                if (!merchantItemCounts[m])
                    merchantItemCounts[m] = {};
